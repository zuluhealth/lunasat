...
import { portalNav, portalSectionIds } from "@/lib/portal/nav";
...
// The portal is one page: on /portal the active item is decided by which
// section is under the header, and nested routes (the airports deck) fall back
// to the section they belong to.
function fallbackActiveHref(pathname: string): string | null {
  if (pathname.startsWith("/portal/sectors")) {
    return "/portal#vertical-solutions";
  }
  return null;
}

const PortalShell: React.FC<PortalShellProps> = ({ email, children }) => {
  const pathname = usePathname() || "";
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [activeSection, setActiveSection] = useState<string>(
    portalSectionIds[0] ?? "",
  );
  const onHubPage = pathname === "/portal";

  // Scroll-spy: mark the section currently sitting under the sticky header.
  useEffect(() => {
    if (!onHubPage) return;

    const elements = portalSectionIds
  ...
  const activeHref = onHubPage
    ? `/portal#${activeSection}`
    : fallbackActiveHref(pathname);
  ...
  return (
    <div className={styles.shell}>
      ...
      <div className={styles.topBar}>
        <Link href="/portal" className={styles.brand} aria-label="Lunasat portal home">
          <svg
            width="100"
            height="12"
          ...
          <span className={styles.brandSuffix}>// PARTNER PORTAL</span>
        </Link>
        ...
        <div className={styles.userBlock}>
          <div className={styles.userMeta}>
            <span className={styles.userMetaLabel}>SESSION</span>
            <span className={styles.userEmail}>{email}</span>
          </div>
          <form action={signOut} className={styles.topSignOutForm}>
            <button type="submit" className={styles.signOut}>
              <TextReveal text="[ SIGN OUT ]" />
            </button>
          </form>
        </div>
      </div>
      ...
      <div className={styles.body}>
        ...
        >
            ...
            {portalNav.map((group) => (
              <div key={group.label} className={styles.navGroup}>
                <div className={styles.navGroupLabel}>
                  <TextReveal text={group.label.toUpperCase()} />
                </div>
                  ...
                  {group.items.map((item) => {
                    ...
                    return (
                      <li key={item.href} className={styles.navItem}>
                        ...
                        >
                          <span className={styles.navLinkMarker}>
                            {active ? "▸" : "·"}
                          </span>
                          {item.index && (
          ...
          <div className={styles.sidebarFooter}>
            <span className={styles.sidebarFooterLine}>// LUNASAT PORTAL</span>
            <span className={styles.sidebarFooterLine}>// v0.1 PREVIEW</span>
            <form action={signOut} className={styles.sidebarSignOutForm}>
