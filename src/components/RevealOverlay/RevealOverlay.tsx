'use client';

import React, { useState, useEffect, useRef } from 'react';
import styles from './RevealOverlay.module.scss';

const RevealOverlay: React.FC = () => {
  const [showText, setShowText] = useState(false);
  const [isRevealing, setIsRevealing] = useState(false);
  const [isComplete, setIsComplete] = useState(false);
  const [radius, setRadius] = useState(30);
  const overlayRef = useRef<HTMLDivElement>(null);
  const animationRef = useRef<number | null>(null);

  useEffect(() => {
    // After 800ms, fade in the text next to the star
    const textTimer = setTimeout(() => {
      setShowText(true);
    }, 800);

    // After text has faded in (800ms + 1000ms fade), start the reveal
    const revealTimer = setTimeout(() => {
      setIsRevealing(true);
    }, 2000);

    return () => {
      clearTimeout(textTimer);
      clearTimeout(revealTimer);
      if (animationRef.current) {
        cancelAnimationFrame(animationRef.current);
      }
    };
  }, []);

  useEffect(() => {
    if (!isRevealing) return;

    const startTime = performance.now();
    const duration = 2800;
    const startRadius = 30;
    const endRadius = 3000;

    const easeOutQuint = (t: number): number => {
      return 1 - Math.pow(1 - t, 5);
    };

    const animate = (currentTime: number) => {
      const elapsed = currentTime - startTime;
      const progress = Math.min(elapsed / duration, 1);
      const easedProgress = easeOutQuint(progress);

      const currentRadius = startRadius + (endRadius - startRadius) * easedProgress;
      setRadius(currentRadius);

      if (progress < 1) {
        animationRef.current = requestAnimationFrame(animate);
      } else {
        setTimeout(() => setIsComplete(true), 100);
      }
    };

    animationRef.current = requestAnimationFrame(animate);

    return () => {
      if (animationRef.current) {
        cancelAnimationFrame(animationRef.current);
      }
    };
  }, [isRevealing]);

  if (isComplete) return null;

  // Reveal from the header logo position (top-left)
  const revealX = typeof window !== 'undefined' && window.innerWidth < 768
    ? (window.innerWidth < 480 ? 48 : 72)
    : 120;
  const revealY = 40;

  const maskStyle = isRevealing ? {
    maskImage: `radial-gradient(
      circle at ${revealX}px ${revealY}px,
      transparent 0px,
      transparent ${radius}px,
      rgba(0, 0, 0, 0.3) ${radius + 20}px,
      rgba(0, 0, 0, 0.7) ${radius + 40}px,
      black ${radius + 60}px
    )`,
    WebkitMaskImage: `radial-gradient(
      circle at ${revealX}px ${revealY}px,
      transparent 0px,
      transparent ${radius}px,
      rgba(0, 0, 0, 0.3) ${radius + 20}px,
      rgba(0, 0, 0, 0.7) ${radius + 40}px,
      black ${radius + 60}px
    )`,
    opacity: radius > 2000 ? Math.max(0, 1 - (radius - 2000) / 1000) : 1,
  } : {};

  return (
    <div
      ref={overlayRef}
      className={styles.overlay}
      style={maskStyle}
    >
      <div className={styles.logoContainer}>
        {/* Star SVG */}
        <svg
          className={styles.star}
          viewBox="0 0 291 131"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M290.462 66.0059C275.693 66.0059 236.347 66.0063 236.347 130.006C236.347 66.0054 236.347 66.0059 0.000342242 66.0058C236.347 66.0059 236.347 66.0059 236.347 2.00587C236.347 66.0059 274.462 66.0059 290.462 66.0059Z" fill="white"/>
        </svg>

        {/* Text SVG */}
        <svg
          className={`${styles.text} ${showText ? styles.textVisible : ''}`}
          viewBox="345 0 753 131"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M355.734 126.95V2.48921H379.204V105.614H436.1V126.95H355.734Z" fill="white"/>
          <path d="M496.634 129.439C486.44 129.439 477.669 127.602 470.32 123.927C463.089 120.134 457.518 114.8 453.607 107.925C449.814 100.931 447.917 92.6934 447.917 83.2107V2.48921H471.387V83.9219C471.387 91.508 473.52 97.494 477.788 101.88C482.173 106.265 488.456 108.458 496.634 108.458C504.813 108.458 511.036 106.265 515.303 101.88C519.689 97.494 521.882 91.508 521.882 83.9219V2.48921H545.352V83.2107C545.352 92.6934 543.396 100.931 539.484 107.925C535.691 114.8 530.12 120.134 522.771 123.927C515.541 127.602 506.828 129.439 496.634 129.439Z" fill="white"/>
          <path d="M568.088 126.95V2.48921H612.716L637.43 110.948H640.631V2.48921H663.745V126.95H619.117L594.402 18.4913H591.202V126.95H568.088Z" fill="white"/>
          <path d="M678.666 126.95L711.381 2.48921H752.275L784.991 126.95H760.81L754.053 99.5683H709.603L702.847 126.95H678.666ZM715.115 77.8767H748.542L733.429 17.2467H730.228L715.115 77.8767Z" fill="white"/>
          <path d="M843.094 129.439C833.493 129.439 825.018 127.72 817.669 124.283C810.319 120.845 804.571 115.926 800.422 109.525C796.273 103.124 794.199 95.4196 794.199 86.4111V81.4327H817.313V86.4111C817.313 93.8787 819.624 99.5091 824.247 103.302C828.87 106.977 835.152 108.814 843.094 108.814C851.154 108.814 857.14 107.214 861.052 104.013C865.082 100.813 867.097 96.7235 867.097 91.7451C867.097 88.3076 866.09 85.5221 864.075 83.3885C862.178 81.2549 859.333 79.5361 855.54 78.2323C851.866 76.8099 847.361 75.506 842.027 74.3206L837.938 73.4316C829.403 71.5351 822.054 69.1644 815.891 66.3196C809.845 63.3563 805.163 59.5039 801.844 54.7626C798.644 50.0212 797.044 43.8575 797.044 36.2713C797.044 28.6852 798.822 22.2251 802.378 16.8911C806.052 11.4385 811.149 7.28983 817.669 4.44502C824.306 1.48167 832.07 0 840.96 0C849.85 0 857.733 1.54094 864.608 4.62282C871.601 7.58616 877.054 12.0904 880.966 18.1357C884.996 24.0623 887.011 31.53 887.011 40.5385V45.8726H863.897V40.5385C863.897 35.7972 862.948 32.0041 861.052 29.1593C859.274 26.196 856.666 24.0623 853.229 22.7585C849.791 21.3361 845.702 20.6249 840.96 20.6249C833.848 20.6249 828.574 21.988 825.136 24.7143C821.817 27.322 820.158 30.9373 820.158 35.5601C820.158 38.642 820.928 41.2497 822.469 43.3833C824.129 45.517 826.559 47.295 829.759 48.7174C832.959 50.1398 837.049 51.3844 842.027 52.4512L846.117 53.3402C855.007 55.2367 862.711 57.6667 869.231 60.63C875.869 63.5933 881.025 67.505 884.699 72.3648C888.374 77.2247 890.211 83.4477 890.211 91.0339C890.211 98.6201 888.255 105.317 884.344 111.125C880.551 116.815 875.098 121.319 867.986 124.638C860.993 127.839 852.695 129.439 843.094 129.439Z" fill="white"/>
          <path d="M899.18 126.95L931.896 2.48921H972.79L1005.51 126.95H981.324L974.568 99.5683H930.118L923.361 126.95H899.18ZM935.63 77.8767H969.056L953.943 17.2467H950.743L935.63 77.8767Z" fill="white"/>
          <path d="M1037.75 126.95V23.8253H1001.48V2.48921H1097.49V23.8253H1061.22V126.95H1037.75Z" fill="white"/>
        </svg>
      </div>
    </div>
  );
};

export default RevealOverlay;
